        ...
        try {
            ...
            while ((headerLine = reader.readLine()) != null && !headerLine.isEmpty()) {
                if (headerLine.toLowerCase().startsWith("host:") ||
                        headerLine.toLowerCase().startsWith("connection:")) {
                    continue;
                }
                accumulatedHeaders.append(headerLine).append("\r\n");
            }

            System.out.println("\n[" + threadName + "] Intercepted Raw Request: " + requestLine);


            String[] parts = requestLine.split(" ");
            if (parts.length < 3 || !parts[0].equalsIgnoreCase("GET")) {
                System.out.println("   -> Error: Only GET requests are supported.");
                clientSocket.close();
                return;
            }
            ...
            System.out.println("   -> Generated Cache Key Target: " + cacheKey);
